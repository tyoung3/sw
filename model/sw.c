...
visitS_tream (S_tream _p_)
{
  ...
  switch (_p_->kind)
    ...
    case is_Streamrx:
      ...
      s =
	MakeStream (state, src, snk, bs, net_model, LatestSrcPort,
		    LatestPort);
  ...
  switch (_p_->kind)
    ...
    case is_Hermtay:
      p = MakeProcess (net_model, "_", NULL,
		   MakeArg (visitListArgument (_p_->u.hermtay_.listargument_),
			    NULL));
      return p;
    default:
      fprintf (stderr, "Error: bad kind field when printing Hermt!\n");
  ...
  switch (_p_->kind)
    ...
    case is_Processx:
      return MakeProcess (net_model,
			  ...
			  MakeArg (visitListArgument
				   (_p_->u.processx_.listargument_), NULL));
    ...
    case is_Processax:		/* Anonymous process */
      ...
      return MakeProcess (net_model,
			  ...
			  c,
			  MakeArg (visitListArgument
				   (_p_->u.processax_.listargument_), name));
      break;
    ...
    case is_Processay:		/* Anonymous process */
      ...
      return MakeProcess (net_model,
			  ...
			  NULL,
			  MakeArg (visitListArgument
				   (_p_->u.processay_.listargument_), name));
    default:
...
visitComp (Comp _p_)
{
  switch (_p_->kind)
    {
    case is_Compx:
      return MakeComponent (visitIdent (_p_->u.compx_.ident_), defaultPath);
    case is_Compy:
