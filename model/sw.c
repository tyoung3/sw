...
/** Get Symbol value */
String
visitSymval (Symval _p_)		 /**<Get symbol value.*/
{
  switch (_p_->kind)
    {
    case is_Symvalv:
...
Extport
visitExtPortOut (ExtPortOut _p_)
{
  Extport ep;
  switch (_p_->kind)
    {
...
/** Get subnet */
Subnetm
visitSubnet (Subnet _p_, Ident id)
{
  Stream s = NULL;
  Extport eport = NULL;
  Process p;
...
/** Get subnet list */
static void
visitListSubnet (ListSubnet listsubnet, Ident id)
{
  while (listsubnet != 0)
    {
...
String
visitValidImport (ValidImport s0)
{
  return s0;
}

Component
visitRemPath (RemPath p)
{
...
visitComp (Comp _p_)
{
  switch (_p_->kind)
    ...
    case is_Compx:
      return MakeComponent (visitSymval (_p_->u.compx_.symval_), defaultPath);
    case is_Compn:
...
ListArgument
visitListArgument (ListArgument listargument)
{
  ListArgument l = NULL;
...
static void
checkIPtype (char *msg, char *srcType, char *snkType, char *srcName,
	     char *snkName)
{
...
static void
Expand3 (Model m, Process p, Extport ep)
{
  ...
  if (ep->type == SOURCE)
      ...
      if (pt)
	{
	  do
	    {
	      pt->id = fixId (pt->id);
	      if ( (pt->id == ep->sink_id) || sameName(pt->name,ep->name) ) 
		{
		  s = pt->stream;
		  VerifyStream (s);
		  ...
		  if (ep->bufsz > s->bufsz)
		    s->bufsz = ep->bufsz;
		  checkIPtype ("Expand3A", ep->iptype, s->iptype,
			       s->source->name, ep->name);
      ...
      if (pt)
	{
	  do
	    {
	      pt->id = fixId (pt->id);
	      if( (pt->id == ep->source_id) || sameName(pt->name,ep->name) )
		{		/* Find matching source port for ep */
		  ...
		  if (ep->bufsz > s->bufsz)
		    s->bufsz = ep->bufsz;
		  checkIPtype ("Expand3B", s->iptype, ep->iptype,
			       s->sink->name, ep->name);
		  if ((ep->iptype && ep->iptype[0]) != 0)
		    s->iptype = ep->iptype;
		  pnew->nportsIn++;
...
static void
Expand (Model m, Process p, Subnetm sn)
{
  Stream s;
  Extport ep;
...
static void
FreeLater (Process * fl, Process p)
{
  p->next = *fl;
  *fl = p;
}
...
static void
FreeExpandedProcesses (Process * fl)
{
  Process pn;
...
static void
createStream (Model m, Extport ep, Extport ep2)
{
  Stream s;
  Port snkpt, srcpt, pt;
...
Model
visitValidSW (Model model, ValidSW _p_)
{
