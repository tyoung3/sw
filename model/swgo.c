...
static void showND(Model m)
{
    ...
    printf("# %d streams, %d processes, %d components, %d partition%s.\n",
	   m->nstreams, m->nprocs, m->ncomponents, nparts, plural);
    printf("*********************************************/\n");
}
...
void genPrefix(Model m)
{
    ...
    P(package main);
    printf("\n/");
    C(*StreamWork Go code. \ nGenerated by swgo.c);
    printf("-%s %s*/\n", version, Timestamp());
    printf("\n");
...
*******************************************************/
 import "fmt"
    ...
    import "github.com/tyoung3/streamwork/poc" func main()
{
    var cs[] chan interface {
    }
    var wg sync.WaitGroup
	fmt.Println("Testgo Start") cs = append(cs, make(chan interface {
							 }
						))

	fbp.Launch(&wg,[]string {
		   "Testgo"}
		   , poc.Print1, cs)
	fbp.Launch(&wg,[]string {
		   "Comp1", "11"}
