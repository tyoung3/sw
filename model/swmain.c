 ...
 if(port&&p->kind==IS_NET) {
  ...
  for (; i < (p->nportsIn + p->nportsOut); i++)
      ...
      if (port->id != i)
	{
      ...
      /*if(portmode==1 && port==port->stream->SinkPort)  {sprintf (fbfr,
		   "(%s) port[%i] is input following output.\n",
		   p->name, i);
		FAIL(badproc, fbfr);
	}
      */
...
static int
nameFail (char *psnkname, char *nsnk, char *nsrc, char *psrcname)
{
...
static void
FixComp (Process p, char *cn, char *path)
{
