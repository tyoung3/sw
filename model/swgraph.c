...
static void genProc(char *name, char *comp, char *path, char *host,
      ...
      printf("  URL=\"%s\"\n", makeURL(comp));
      printf(" host=\"%s\" \n", host);
      printf(" tooltip=\"%s.%s ", path, comp); genArgs(args);
      printf("\"\n");

      switch (showcomp) {
      case NOCOMP:
      		printf("label=\"{<P> %s ", name); break;
      case COMPNAME:printf("label=\"{<P> %s %s %s", name, twoline,
			   comp); genArgs(args); break;
      case WITHPATH:printf("label=\"{<P> %s %s %s.%s", name, twoline,
...
static int findChannel(Port p, int id)      {
      ...
      if (p->id == id) {
      return p->channel;}
      p = p->next;} while (p != p0); return -1;
}

static void showPorts(Stream f, Process src, Process snk,
			    int channel) {
#ifndef NO_PORTS
      printf
      ...
      Stream f; Process src, snk; int channel = 7; f = m->stream; 
...
void genGraph(Model model) {
      ...
      genPrefix(model->name, model->nstreams); genCluster1(model->name); p = model->proc;
	/* Get first process */
      genProcs(p);   /*{*/
      P(});			/* End Cluster1 */
