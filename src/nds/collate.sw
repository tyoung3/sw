...
(C swutility/Collate ATTRIBUTES { size="large", fill="lightgreen"}) 
     <1-  (G0  Gen "12" "1" "2" );  
(C ATTRIBUTES { test="yes" })1	    		<2- 0(M   swutility/Merge);      
(M)2	    		<int-  (G1  Gen "5"  "5" "1" );
(M)IN.1		    	<int 3-  (G2  Gen "8"  "2" "3" );

(Match0 Print ATTR {fillcolor="lightgreen", tooltip=$tip }) 	<int- 2(C);
...
(Miss1  Print attr {tooltip="MISS!!", fillcolor="red"}) 	<int- OUT.5(C);   
